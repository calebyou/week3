...
def pick_random_movie(formatted_movies):
    # Split the formatted movies string into individual movie entries
    movies = formatted_movies.strip().split('\n\n')

    # List to hold parsed movie information
    movie_list = []

    # Extract the title and movie ID from each entry
    for movie in movies:
        lines = movie.split('\n')
        title = None
        movie_id = None
        ...
        for line in lines:
            ...
            elif line.startswith("**Movie ID:**"):
                movie_id = line.split("**Movie ID:**")[1].strip()
    ...
    if movie_list:
        ...
        return selected_movie['title'], selected_movie['movie_id']
    else:
        return None, None
...
def get_now_playing_movies():
    ...
    headers = {
        ...
        "Authorization": f"Bearer {os.getenv('TMDB_API_ACCESS_TOKEN_')}"
    }
